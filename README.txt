...
------------------------------------------------------------------------------------------------------
** TODO: Create MySQL server locally from script.

For a temporary fix:
Both the python script and java code connect to a specific MySQL server used during implementation. 

Inside the java code, there is a DatabaseManager.class file. That class sets up the specific database
(using localhost as the host, root as the user, and z*>L?yZc=3Z%b9b8R,9e4V@X as the password). These 
values would be changed depending on the database server to connect to.
